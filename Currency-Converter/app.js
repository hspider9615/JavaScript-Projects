...
const selectPair = (e) => {
  if (e.target.hasAttribute('data-code')) {
    ...
    // load the exchange rates then update the buttons
    loadExchangeRate();
...
const updateInputs = () => {
  ...
  const result = baseValue * rates[target];
...
const updateExchangeRate = () => {
  ...
  const rate = rates[target].toFixed(4);
...
const fetchCurrencies = () => {
  fetch(`https://v6.exchangerate-api.com/v6/${apiKey}/codes`)
    ...
    .then(({ supported_codes }) => {
      state.currencies = supported_codes.map(([code, name]) => ({
        code,
        name,
      }));
